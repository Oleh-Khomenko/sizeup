...
async function bundlePackage(pkgDir, pkg) {
  ...
  const isCss = /\.css$/.test(entryPoint);
  ...
  const baseOpts = {
    ...
    logLevel: 'silent',
    external: peerDeps,
    define: isCss ? undefined : { 'process.env.NODE_ENV': '"production"' },
  };
  ...
  const minResult = await esbuild.transform(raw, { minify: true, loader: isCss ? 'css' : 'js' });
...
function resolveExport(exports) {
  if (typeof exports === 'string') {
    return /\.(d\.ts|d\.mts|scss|less)$/.test(exports) ? null : exports;
  }
