...
function install(spec, opts = {}) {
  ...
  try {
    execSync(`npm install --ignore-scripts --production${forceFlag} ${spec}`, {
      cwd: tmpDir,
      timeout: 60000,
      stdio: 'pipe',
    });
...
function installAsync(spec, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    const forceFlag = opts.force ? ' --force' : '';
    exec(`npm install --ignore-scripts --production${forceFlag} ${spec}`, {
      cwd: tmpDir,
      timeout: 60000,
    }, (err, _stdout, stderr) => {
      if (err) {
...
function installBatchAsync(specs, onProgress, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    exec(`npm install --ignore-scripts --production${forceFlag} ${escaped}`, {
      cwd: tmpDir,
      timeout: 120000,
    }, (err, _stdout, stderr) => {
