...
function install(spec) {
  ...
  try {
    execSync(`npm install --ignore-scripts --production ${spec}`, {
      cwd: tmpDir,
      timeout: 60000,
      stdio: 'pipe',
    });
...
function installAsync(spec) {
  return new Promise((resolve, reject) => {
    ...
    exec(`npm install --ignore-scripts --production ${spec}`, {
      cwd: tmpDir,
      timeout: 60000,
    }, (err, _stdout, stderr) => {
      if (err) {
...
function installBatchAsync(specs, onProgress) {
  return new Promise((resolve, reject) => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sizeup-'));
    trackDir(tmpDir);
    ...
    const escaped = specs.map(s => `"${s}"`).join(' ');
    exec(`npm install --ignore-scripts --production ${escaped}`, {
      cwd: tmpDir,
      timeout: 120000,
    }, (err, _stdout, stderr) => {
